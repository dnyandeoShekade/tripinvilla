...
export default function PropertyMakers() {
  ...
  const handleRuleSectionChange = (idx, field, val) => {
    const arr = [...rulesSections];
    arr[idx][field] = val;
    setRulesSections(arr);
  };
  ...
  const fetchExperiences = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE}/master/experiences/active`,
      );
      const data = await res.json();
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <button
              ...
              style={{ padding: '4px 12px', background: '#F3F4F6', border: '1px solid #D1D5DB', borderRadius: '6px', fontSize: '12px', fontWeight: 600, color: '#374151', cursor: 'pointer' }}
            >
              + Add Section
            </button>
          </div>
          ...
          {rulesSections.map((sec, idx) => (
            <div key={idx} style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '8px', padding: '14px', marginBottom: '12px', position: 'relative' }}>
              {rulesSections.length > 1 && (
                <button
                  type="button"
                  onClick={() => handleRemoveRuleSection(idx)}
                  style={{ position: 'absolute', top: '10px', right: '10px', background: '#FEE2E2', color: '#EF4444', border: 'none', borderRadius: '4px', width: '22px', height: '22px', cursor: 'pointer', fontSize: '14px', fontWeight: 'bold' }}
                >
                  &times;
                </button>
              )}
              <input
                type="text"
                placeholder="Section Title (e.g. Must Read Rules)"
                className="form-input"
                value={sec.title}
