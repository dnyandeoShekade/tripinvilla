...
export default function RoomTypeMaster() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        const res = await fetch(`${import.meta.env.VITE_API_BASE}/master/room-types/${formData.id}`, {
          method: 'PUT',
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE}/master/room-types/${deleteTargetId}`, { method: 'DELETE' });
      if (res.ok) fetchTypes();
  ...
  return (
    <div className="fade-in">
      <div className="props-breadcrumb" style={{ margin: '0 39px 12px' }}>
        Masters &gt; <span>Room Type Master</span>
      </div>
        ...
        <form onSubmit={handleSubmit} className="master-form-card" style={{ margin: 0 }}>
          <div className="master-form-header">
            <div className="master-form-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {isEditing ? 'Modify Room Type' : 'Add New Room Type'}
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '16px', maxWidth: '400px', width: '100%', padding: '24px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', border: '1px solid #E5E7EB', margin: 'auto' }}>
            <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
              ...
              <div>
                <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#111827', margin: 0 }}>Delete Room Type</h3>
